...
echo "run-id=issue-<番号>-iterate-${RUN_TS}"
mkdir -p "docs/experiments/pr-strategy/issue-<番号>-iterate-${RUN_TS}"
```
...
- `docs/experiments/pr-strategy/<run-id>/metrics.json` を Write（README の metrics スキーマ、`elapsed_source: "pr_records"`）。
