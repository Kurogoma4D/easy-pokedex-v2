...
describe('fetchPokemonDetail', () => {
  ...
  it('selects the latest cry url (cries.latest preferred)', async () => {
    const client = makeClient(makeFetchImpl());
...
describe('selectCryUrl', () => {
  function pokemonWith(cries: PokeApiPokemon['cries']): PokeApiPokemon {
    return {
      id: 1,
      name: 'x',
      height: 1,
  ...
  it('prefers cries.latest when present', () => {
    expect(selectCryUrl(pokemonWith({ latest: 'latest.ogg', legacy: 'legacy.ogg' }))).toBe(
      'latest.ogg',
    );
  });

  it('falls back to cries.legacy when latest is missing', () => {
    expect(selectCryUrl(pokemonWith({ latest: null, legacy: 'legacy.ogg' }))).toBe('legacy.ogg');
    expect(selectCryUrl(pokemonWith({ legacy: 'legacy.ogg' }))).toBe('legacy.ogg');
  });

  it('returns null when neither source nor the cries object exists', () => {
    expect(selectCryUrl(pokemonWith({ latest: null, legacy: null }))).toBeNull();
    expect(selectCryUrl(pokemonWith(undefined))).toBeNull();
  });
});
