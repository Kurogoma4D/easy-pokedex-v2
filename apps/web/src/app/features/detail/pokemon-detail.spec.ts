...
describe('PokemonDetail', () => {
  ...
  it('enables the cry button and plays the cry url when clicked', async () => {
    const fixture = createFixture('1');
    await flushDetail(fixture);
    const el = fixture.nativeElement as HTMLElement;
    ...
    // ラベルは「再生」文言（音源あり）。
    expect(button.getAttribute('aria-label')).toContain('さいせい');

    // クリックで Audio.play が呼ばれる。jsdom は実再生しないため play をスパイして検証する。
  ...
  it('localizes the cry button label when the locale changes', async () => {
    const fixture = createFixture('1');
    await flushDetail(fixture);
    const el = fixture.nativeElement as HTMLElement;
    const button = el.querySelector('.detail__cry') as HTMLButtonElement;
  ...
  it('disables the cry button and labels it unavailable when there is no cry url', async () => {
    const fixture = createFixture('1');
    ...
    expect(button.getAttribute('aria-label')).toContain('ありません');

    TestBed.inject(LocaleService).setLocale('en');
    await render(fixture);
    expect(button.getAttribute('aria-label')).toBe('No cry available');
  });
